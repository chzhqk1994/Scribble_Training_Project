...
def json_parser(file_path, change_label_dict):
    with open(file_path) as json_file:
        ...
        # change_label_dict = {'wall': 'concrete'}  # key: value, change 'key' to 'value'

        change_class_list = list(change_label_dict.keys())

        for shape in shapes:
            if shape["label"] in change_class_list:  # 이름 변경 대상인 경우
                label_points = {}
                label_points["label"] = change_label_dict[shape["label"]]
...
def walk_around_files(change_label_dict, src_anno_dir_list, dst_anno_dir_list, src_img_dir_list, dst_img_dir_list):
    err_file_cnt = 0
    for src_dir in src_anno_dir_list:
        ...
        for root, dirs, files in os.walk(src_dir):
            ...
            for file in files:
                ...
                new_json = json_parser(current_anno_file, change_label_dict)
...
if __name__ == "__main__":
    ...
    separated_dir = '/home/qisens/dataset/roof_segment_augment/wall_to_concrete/'

    src_anno_dir_list, dst_anno_dir_list, src_img_dir_list, dst_img_dir_list = make_dir_list(src_dir, separated_dir)
    walk_around_files(change_label_dict, src_anno_dir_list, dst_anno_dir_list, src_img_dir_list, dst_img_dir_list)
