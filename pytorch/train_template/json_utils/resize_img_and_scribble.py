...
def json_parser(file_path, key, resize_ratio):
    ...
    for src_dir in src_anno_dir_list:
        ...
        for root, dirs, files in os.walk(src_dir):
            ...
            for file in files:
                ...
                #key = ['bg','tree','flat','road','parkinglot','ground','grass','slope','river','yellowgrass', 'cemetery', 'outerwall']
                key = ['bg', 'waterproof', 'facility', 'something', 'airconditioner', 'wall', "garden", "corrugated", "slate", "solarpanel", "heliport", "tree", "concrete", "clay"]
                #key = ['ground','river']
                #key = ['bg','flat','road','parkinglot','ground','grass','slope','river']

                new_json = json_parser(current_anno_file, key, resize_ratio)
                if new_json :
                # To Do
                    try:
...
if __name__ == "__main__":
    ...
    separated_dir = "/media/qisens/4tb3/navermap_detection/dataset/scribble_dataset/building_capture/augmentation/resize_" + str(resize_ratio)
